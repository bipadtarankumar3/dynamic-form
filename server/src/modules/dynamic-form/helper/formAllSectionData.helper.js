...
function buildSelectQueryById({ schema, selected_data = {} }) {
      ...
      section?.fields?.forEach((f) => {
        ...
        if (f?.type === "file") {
          ...
          documentAggs.push(`
            ...
            FROM ${doc.table} d
            WHERE CAST(d.${doc.foreign_key} AS TEXT) = CAST(${sectionAlias}.${section.primary_key} AS TEXT)
        ...
        section?.fields?.forEach((f) => {
          if (f?.type === "file") {
            ...
            documentAggs.push(`
              ...
              FROM ${doc.table} d
              WHERE CAST(d.${doc.foreign_key} AS TEXT) = CAST(r.${rootPK} AS TEXT)
                AND d.${doc.doc_type_key} = '${f.db_field}'
                AND d.deleted_at IS NULL
            `);
          }
      ...
      section.fields.forEach((f) => {
        ...
        if (f.type === "file") {
          ...
          rowDocAggs.push(`
            ...
            FROM ${doc.table} d
            WHERE CAST(d.${doc.foreign_key} AS TEXT) = CAST(${childAlias}.${section.primary_key} AS TEXT)
              AND d.${doc.doc_type_key} = '${f.db_field}'
              AND d.deleted_at IS NULL
          `);
          return;
  ...
  const targetId =
    (typeof selected_data === "object" && selected_data !== null
      ? selected_data[rootPK] ?? selected_data.id ?? Object.values(selected_data)[0]
      : selected_data) ?? null;

  whereClause.push(`CAST(r.${rootPK} AS TEXT) = CAST(:${rootPK} AS TEXT)`);
  replacements[`${rootPK}`] = targetId !== null && targetId !== undefined ? String(targetId) : "";
...
const { getDbMasterConfig } = require("./masterResolver");
