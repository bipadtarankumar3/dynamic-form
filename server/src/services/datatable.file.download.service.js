...
class DatatablesFileDownload {
   ...
   * @param {Object} bindParams - Bind parameters for the query.
   ...
   */
  static async build(req, sql, whereClause = "", bindParams = {}, callback = null, groupBy = "") {
    ...
    // Build global search
    let searchQuery = this.buildSearchQuery(searchValue, columns, bindParams).trim();
  ...
  static buildSearchQuery(searchValue, columns, bindParams) {
    ...
    columns.forEach((column, index) => {
      ...
      if ((column.searchable === true || column.searchable === "true") && colName) {
        ...
        bindParams[bindKey] = `%${trimmedSearch}%`;
      }
    });
...
module.exports = DatatablesFileDownload;
