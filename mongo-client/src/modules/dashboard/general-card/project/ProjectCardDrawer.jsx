...
import { getProjectCountThemeWiseAPI } from "@/services/dashboard-service";

const { Panel } = Collapse;
import { Tabs } from "antd";
import ThemeStatusRadialChart from "./ThemeStatusRadialChart";
const { TabPane } = Tabs;
const ProjectCardDrawer = ({ open, onClose }) => {
  ...
  const fetchThemes = async (filterValues) => {
    try {
      setLoading(true);
      const res = await getProjectCountThemeWiseAPI(filterValues);
  ...
  const buildPayload = useMemo(
    () => ({
      ...
      columns: [
        ...
        {
          data: "tprjct_project_duration_start_date",
          name: "tprjct_project_duration_start_date",
          searchable: "true",
          orderable: "true",
          search: { value: "", regex: false },
        },
        {
          data: "tprjct_project_duration_end_date",
          name: "tprjct_project_duration_end_date",
          searchable: "true",
          orderable: "true",
          search: { value: "", regex: false },
        },
        {
          data: "tprjct_project_budget",
          name: "tprjct_project_budget",
          searchable: "true",
          orderable: "true",
          search: { value: "", regex: false },
        },
        {
          data: "tprjct_created_by",
          name: "tprjct_created_by",
          searchable: "true",
          orderable: "true",
          search: { value: "", regex: false },
        },
  ...
  const handleExcelDownload = async (theme_name) => {
    ...
    try {
      const response = await privateHttpClient.post(
        "dash/project-by-theme-file-download",
        buildPayload,
        {
      ...
      let fileName = `${theme_name} Projects List.xlsx`;
          ...
          {loading ? (
            ...
            >
              {themes?.length > 0 ? (
                themes?.map((theme) => (
                  <Panel
                    header={`${theme?.tthm_name_of_theme} (${theme?.total_project})`}
                    key={theme?.tthm_id}
                  >
                    {/* SEARCH + DOWNLOAD */}
                          ...
                          proposalData?.[theme?.tthm_id]?.data === undefined ? (
                            ...
                            proposalData?.[theme?.tthm_id]?.data?.map((row) => (
                              <tr key={row?.tprjct_id}>
                                ...
                                <td className="border px-2">
                                  {row?.tprjct_project_budget
                                    ? row?.tprjct_project_budget?.toLocaleString()
                                    : ""}
                                </td>
                                <td className="border px-2">
                                  {row?.created_by_name || ""}
