...
const ThemeStatusRadialChart = ({ data = [] }) => {
  ...
  const pendingSeries = data?.map((t) => Number(t?.total_pending_project || 0));

  const approvedSeries = data?.map((t) =>
    Number(t?.total_approved_project || 0),
  );

  const resendSeries = data?.map((t) => Number(t?.total_resend_project || 0));
  const maxCount = Math.max(...data.map((t) => Number(t?.total_project || 0)));
  ...
  const options = {
    ...
    title: {
      text: "Theme Wise Project Status",
    },
