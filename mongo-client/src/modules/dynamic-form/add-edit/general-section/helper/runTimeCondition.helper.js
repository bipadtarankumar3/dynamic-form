  ...
  export const getFieldRuntimeState = (field, values, role = "admin") => {
    const visible =
      !field.visible_when || evaluateCondition(field.visible_when, values);

    const enabled =
      !field.enabled_when || evaluateCondition(field.enabled_when, values);

    const disabled =
      field.disabled === true ||
      (field.disabled_when && evaluateCondition(field.disabled_when, values));
