...
/**
 * DdFormModal.jsx
 * Due Diligence specific form modal — completely independent of NgoDynamicFormModal.
 * Hardcodes DD constants (form_slug, API endpoint, title) so the DD page
 * can evolve separately with DD-specific conditions and fields.
 */
...
export default function DdFormModal({
  open,
  onClose,
  schema,
  initialValues = {},
        ...
        if (sec.type === "add_more") {
          ...
          if (isMasterDriven && masterSource) {
            try {
              ...
              if (Array.isArray(masterItems) && masterItems.length > 0) {
                ...
                STATUTORY_ITEMS.forEach((stat) => {
                  ...
                  const found = masterItems.find((item) =>
                    normalize(item.label || item.type_name || item.name) === normStat
                  );
                  matched.push(found || { id: stat, type_name: stat, label: stat });
                });
                const targetItems = matched.length > 0 ? matched : masterItems;

                const existingMap = new Map();
                if (Array.isArray(rows)) {
                  rows.forEach((r) => {
                    const k = String(
                      r.registration || r.registration_name || r.document_type ||
                      r.type_name    || r.name || ""
                    ).toLowerCase().trim();
              ...
              console.warn("[DdFormModal] master load error:", masterErr);
            }
          }
          ...
          if (Array.isArray(rows)) {
            const processedRows = rows.map((r) => {
              ...
              (sec.fields || []).forEach((f) => {
                ...
                if (f.type === "date" && rCopy[k]) rCopy[k] = dayjs(rCopy[k]);
                if (["file", "upload", "document"].includes(String(f.type || "").toLowerCase())) {
                  const val = rCopy[k];
          ...
          (sec.fields || []).forEach((f) => {
            ...
            if (f.type === "date" && formatted[key]) formatted[key] = dayjs(formatted[key]);
          });
        }
      }

      if (isMounted) form.setFieldsValue(formatted);
    };

    setupFormValues();
    return () => { isMounted = false; };
  }, [open, initialValues, schema, form]);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const payload = { form_slug: DD_FORM_SLUG };
      if (values.id || initialValues?.id) payload.id = values.id || initialValues.id;

      for (const [k, v] of Object.entries(values)) {
        if (v === undefined || v === null) { payload[k] = v; continue; }
        if (dayjs.isDayjs(v)) { payload[k] = v.format("YYYY-MM-DD"); continue; }

        if (Array.isArray(v)) {
          if (v.length === 0) { payload[k] = []; continue; }
          const first = v[0];
          if (first && typeof first === "object" && (first.uid !== undefined || first.originFileObj)) {
      ...
      const res = await privateHttpClient.post(DD_API_ENDPOINT, payload, {
        headers: { "Content-Type": "application/json" },
      });

      if (res?.data?.success) {
        message.success(res.data.message || "Due Diligence saved successfully!");
        if (onSuccess) await onSuccess(res.data);
        onClose();
      } else {
        message.error(res?.data?.message || "Failed to save Due Diligence details");
      }
    } catch (err) {
      console.error("[DdFormModal] save error:", err);
  ...
  const renderFieldInput = (field) => {
    ...
    if (type === "number" || type === "currency")
      return (
        <InputNumber
          ...
          formatter={type === "currency" ? (v) => `₹ ${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",") : undefined}
        />
      );
    ...
    if (type === "select" || type === "dropdown") {
      const opts = Array.isArray(field.options)
        ? field.options.map((o) =>
            typeof o === "string" ? { label: o, value: o } : { label: o.label || o.name || o.value, value: o.value ?? o.id }
          )
        : [];
      return <Select placeholder={`Select ${field.label || field.name}`} options={opts} allowClear showSearch optionFilterProp="label" />;
    }
    ...
    if (type === "multiselect" || type === "multi_select" || type === "tags") {
      const opts = Array.isArray(field.options)
        ...
        : [];
      return <Select mode="multiple" placeholder={`Select ${field.label || field.name}`} options={opts} allowClear />;
    }
      ...
      return (
        <Space direction="vertical" style={{ width: "100%" }}>
          <Upload
            ...
            onChange={({ fileList }) => setFileListMap((prev) => ({ ...prev, [fieldKey]: fileList }))}
          >
    ...
    <Modal
      title={
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <SafetyOutlined style={{ color: "#ffffff", fontSize: 18 }} />
          <span style={{ fontWeight: 700, fontSize: 17, color: "#ffffff" }}>
            {mode === "add" ? `Add ${DD_TITLE}` : `Edit ${DD_TITLE}`}
          </span>
        </div>
      }
      open={open}
      ...
      destroyOnHidden
      style={{ maxWidth: "94vw", top: 18, paddingBottom: 20 }}
    >
      <Form
        form={form}
        layout="vertical"
        onFinish={handleSubmit}
        style={{ display: "flex", flexDirection: "column", maxHeight: "calc(88vh - 70px)" }}
      >
        <div
          style={{ overflowY: "auto", overflowX: "hidden", padding: "16px 4px 10px 4px", flex: "1 1 auto" }}
            ...
            if (isAddMore) {
              ...
              return (
                <Card
                  ...
                  style={{ borderRadius: 10, border: "1px solid #e2e8f0", marginBottom: 16, background: "#ffffff" }}
                      ...
                      <>
                        {isMasterDriven ? (() => {
                          ...
                          const renderTableCellInput = (fld, rowName, restF) => {
                            ...
                            if (t === "select") {
                              const opts = Array.isArray(fld.options)
                                ? fld.options.map((o) => typeof o === "string" ? { label: o, value: o } : { label: o.label || o.name || o.value, value: o.value ?? o.id })
                                : [];
                              return (
                                <Form.Item {...restF} name={[rowName, fKey]} style={{ margin: 0 }}>
                                  <Select size="middle" style={{ width: "100%", minWidth: 110 }} placeholder={fld.ui?.placeholder || `Select ${fld.label || fld.name}`} options={opts} allowClear />
                                </Form.Item>
                              );
                            }
                            if (t === "date") {
                              return (
                                <Form.Item {...restF} name={[rowName, fKey]} style={{ margin: 0 }}>
                                  <DatePicker size="middle" style={{ width: "100%", minWidth: 130 }} format="YYYY-MM-DD" placeholder={fld.ui?.placeholder || "Select Date"} />
                                </Form.Item>
                              );
                            }
                            if (["file", "upload", "document"].includes(t)) {
                              return (
                                <Form.Item {...restF} name={[rowName, fKey]} valuePropName="fileList" getValueFromEvent={normFile} style={{ margin: 0 }}>
                            ...
                            if (t === "textarea") {
                              return (
                                <Form.Item {...restF} name={[rowName, fKey]} style={{ margin: 0 }}>
                                  <TextArea size="middle" rows={1} autoSize={{ minRows: 1, maxRows: 3 }} placeholder={fld.ui?.placeholder || `Enter ${fld.label || fld.name}`} style={{ width: "100%", minWidth: 130 }} />
                                </Form.Item>
                              );
                            }
                            if (["switch", "boolean", "checkbox"].includes(t)) {
                              return (
                                <Form.Item {...restF} name={[rowName, fKey]} valuePropName="checked" style={{ margin: 0, textAlign: "center" }}>
                                  <Switch size="small" />
                                </Form.Item>
                              );
                            }
                            return (
                              <Form.Item {...restF} name={[rowName, fKey]} style={{ margin: 0 }}>
                                <Input size="middle" placeholder={fld.ui?.placeholder || `Enter ${fld.label || fld.name}`} style={{ width: "100%", minWidth: 120 }} />
                              </Form.Item>
                            );
                          };

                          return (
                            <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch", borderRadius: 8, border: "1px solid #e2e8f0", background: "#ffffff" }}>
                              <table style={{ width: "100%", minWidth: Math.max(780, (dataFields.length + 1) * 135), borderCollapse: "separate", borderSpacing: 0 }}>
                                <thead>
                                  <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                                    ...
                                    {dataFields.map((fld) => (
                                      <th key={fld.field_id || fld.id || fld.db_field || fld.slug} style={{ padding: "12px 12px", fontSize: 13, fontWeight: 700, color: "#1e293b", textAlign: "left" }}>
                                        {fld.label || fld.title || fld.name}
                                        {fld.required && <span style={{ color: "#ef4444", marginLeft: 4 }}>*</span>}
                                      </th>
                                    ))}
                                    {allowDelete && (
                                      <th style={{ padding: "12px 10px", fontSize: 13, fontWeight: 700, color: "#1e293b", textAlign: "center", width: 60 }}>Action</th>
                                    )}
                                  </tr>
                                </thead>
                                <tbody>
                                  {fieldsList.map(({ key, name, ...restField }, rIdx) => {
                                    ...
                                    const docLabel =
                                      rowVal[labelKey] || rowVal.registration || rowVal.registration_name ||
                                      rowVal.type_name || rowVal.document_type || rowVal.name ||
                                      `Compliance Item #${rIdx + 1}`;

                                    return (
                                      <tr key={key} style={{ background: rIdx % 2 === 0 ? "#ffffff" : "#fcfcfd", borderBottom: "1px solid #f1f5f9" }}>
                                        <td style={{ padding: "10px 16px", fontWeight: 600, fontSize: 13, color: "#1e293b", verticalAlign: "middle" }}>
                                          <Form.Item {...restField} name={[name, "id"]} hidden><Input type="hidden" /></Form.Item>
                                          {labelCol && (<Form.Item {...restField} name={[name, labelKey]} hidden><Input type="hidden" /></Form.Item>)}
                                          <Form.Item {...restField} name={[name, "registration"]} hidden><Input type="hidden" /></Form.Item>
                                          <Form.Item {...restField} name={[name, "registration_name"]} hidden><Input type="hidden" /></Form.Item>
                                          <Form.Item {...restField} name={[name, "type_name"]} hidden><Input type="hidden" /></Form.Item>
                                          <span>{docLabel}</span>
                                        </td>
                                        {dataFields.map((fld) => (
                                          <td key={fld.field_id || fld.id || fld.db_field || fld.slug} style={{ padding: "8px 10px", verticalAlign: "middle" }}>
                          ...
                          fieldsList.map(({ key, name, ...restField }, rIdx) => (
                            <Card
                              ...
                              extra={
                                allowDelete && (
                                  <Button type="text" danger icon={<DeleteOutlined />} onClick={() => remove(name)}>Remove</Button>
                                )
                              }
                            >
                              <Form.Item {...restField} name={[name, "id"]} hidden><Input type="hidden" /></Form.Item>
                                  ...
                                  return (
                                    <Col xs={24} sm={isFull ? 24 : 12} md={isFull ? 24 : (fields.length > 2 && fields.length % 3 === 0 ? 8 : 12)} key={f.field_id || `add-${fIdx}`}>
                                      <Form.Item
                                        ...
                                        valuePropName={["switch","boolean","checkbox"].includes(String(f.type||"").toLowerCase()) ? "checked" : ["file","upload","document"].includes(String(f.type||"").toLowerCase()) ? "fileList" : undefined}
                                        getValueFromEvent={["file","upload","document"].includes(String(f.type||"").toLowerCase()) ? normFile : undefined}
                                        label={<span>{f.label || f.title || f.name}{f.required && <span style={{ color: "#ef4444", marginLeft: 2 }}>*</span>}</span>}
                        ...
                        {allowAdd && (
                          <Button type="dashed" onClick={() => add()} icon={<PlusOutlined />} style={{ width: "100%", marginTop: 8, borderRadius: 6 }}>
                            Add Row
                          </Button>
                        )}
                      </>
        ...
        {/* Sticky footer */}
        <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 12, padding: "14px 16px 6px 16px", borderTop: "1px solid #e2e8f0", background: "#ffffff", flexShrink: 0 }}>
          <Button onClick={handleModalClose} disabled={submitting} size="middle" style={{ borderRadius: 6, minWidth: 90, fontWeight: 600 }}>
            Cancel
          </Button>
          <Button
            type="primary"
            ...
            loading={submitting}
            size="middle"
            style={{ background: "#15803d", borderColor: "#15803d", fontWeight: 700, borderRadius: 6, minWidth: 170, boxShadow: "0 2px 8px rgba(21, 128, 61, 0.25)" }}
          >
            Submit New Version
          </Button>
        </div>
