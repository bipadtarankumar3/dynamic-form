...
const StudentCoverageDrawer = ({ open, onClose }) => {
  ...
  const fetchNgoList = useCallback(async () => {
    try {
      ...
      message.error("Failed to load student coverage list");
    } finally {
      setLoading(false);
    }
  }, [buildPayload]);
  ...
  const handleExcelDownload = async () => {
    try {
      const response = await privateHttpClient.post(
        "dash/student-coverage-file-download",
        buildPayload,
        {
      ...
      let fileName = "Project Wise Students Coverage List.xlsx";
  ...
  return (
    <Drawer
      title="Project Wise Students Coverage"
      placement="right"
      onClose={onClose}
      open={open}
      width={900}
        ...
        <table className="min-w-full border text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="border px-2">Male</th>
              <th className="border px-2">Female</th>
            ...
            {loading ? (
              ...
              records.map((row) => (
                <tr key={row?.tng_id}>
                  ...
                  <td className="border px-2">{row?.male_count}</td>
                  <td className="border px-2">{row?.female_count}</td>
...
export default StudentCoverageDrawer;
