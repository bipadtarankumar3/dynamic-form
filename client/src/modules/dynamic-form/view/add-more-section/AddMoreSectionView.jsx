...
const AddMoreSectionView = ({ data = [], section, isActiveKey, allData = {} }) => {

  // Helper to identify numeric/financial fields
  ...
  // Helper to identify currency/financial fields
  ...
  const getValue = (field, row) => {
    ...
    // Select → Parent & Child dependency resolution + *_name_* lookup
    if (field?.type === "select") {
      const nameKey = Object.keys(row).find(
        (k) => k.endsWith(`_${field?.db_field}`) && k !== field?.db_field
      );
      ...
      let parentName = parentNameKey ? row[parentNameKey] : parentValue;
  ...
  // Calculate field numeric total safely across data rows
  ...
  // Financial KPI totals for stat_widgets view
  ...
  return (
    ...
    >
      {/* KPI Stat Widgets (if display_mode === 'stat_widgets') */}
      ...
      {/* Render Card Grid mode if display_mode === 'cards' */}
      {displayMode === "cards" ? (
...
export default React.memo(AddMoreSectionView);
